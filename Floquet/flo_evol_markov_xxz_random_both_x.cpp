//
// Created by Liangsheng Zhang on 3/14/15.
//
...
/**
 ** Implementation of FloEvolMarkovXXZRandomBoth class.
 **/

/*
 * Construct the representation string and abstract type of the class.
 */
void FloEvolMarkovXXZRandomBothX::Repr_Init_(){
	repr_ << "Markov_XXZ_Random_Both_X_Floquet_L=" << size_ << ",lambda=" << param_.lambda
			<< ",K=" << param_.K << ",tau="
		  << param_.tau;
	type_ = "Markov_XXZ_Random_Both_X_Floquet";
}

/*
 * Initialize the map linking name and position of evol_op
 */
void FloEvolMarkovXXZRandomBothX::Op_Name_Init_(){
	op_name_["down"] = 0;
	op_name_["Down"] = 0;
	op_name_["up"] = 1;
	op_name_["Up"] = 1;
 ...
 */
void FloEvolMarkovXXZRandomBothX::Evol_Construct(){
	...
	FloEvolXXZRandom* floquet_iso = new FloEvolXXZRandom(param_.size, param_.tau,
		param_.g, param_.h, param_.lambda, debug_);
 ...
 */
void FloEvolMarkovXXZRandomBothX::Bath_Construct_(MatrixXcd& U, string type){
...
void FloEvolMarkovXXZRandomBothX::Eigen_Name_Construct_(){

...
/*
 * Compute entry and norm representations of the left sigma matrix at the end of chain
 * for floquet systems.
 */
...
void flo_leftmost_sigma_z(const AllPara& parameters){
	...
	for (int i=0; i< num_realization; i++){
		...
		if (!output_init){
			...
			base_name_m<<",Realizations="<<num_realization<<",leftmost_sigma_z";
		...
		leftmost_sigma_z_sum(sigma_z_entry, evec_basis, "entry");
		...
		leftmost_sigma_z_sum(sigma_z_norm, evec_basis, "norm");
